...
class ForescoutCounteractConnector(BaseConnector):
    ...
    def _make_rest_call(self, module, endpoint, action_result, method="get", auth=None, headers=None, **kwargs):
        ...
        if module == "web":
            ...
            headers = {"Authorization": token, "Accept": "application/ha1+json"}
        ...
        try:
            ...
            error_message = f"Error connecting to server. Connection Refused from the Server for invalid URL {url}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, error_message), resp_json)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
    ...
    def _handle_test_connectivity(self, param):
        ...
        if web_credentials:
            ...
            self.save_progress(f"Connecting to endpoint {FS_WEB_HOSTS} to test Web API connectivity")
